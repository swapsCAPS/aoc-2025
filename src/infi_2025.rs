...
/*
 * Using zip here has the nice benefit that it automagically caps the ranges by x or y
 */
...
pub fn check_fluorescence(
    ...
    let own_height = line[0].1.height.unwrap_or_default();
    ...
    let gets_light = line
        ...
        .any(|(pos, fluor_tree)| {
            let fluor_tree_height = fluor_tree.1.height.unwrap_or_default();
            if fluor_tree_height == 0 {
                return false;
            }

            line[1..pos].iter().all(|blocking_tree| {
                let blocking_tree_height = blocking_tree.1.height.unwrap_or_default();
                if blocking_tree_height == 0 {
                    return true;
                }
                blocking_tree_height < fluor_tree_height && blocking_tree_height < own_height
            })
        });
...
mod tests {
    ...
    fn gets_fluorescence() {
        ...
        assert_eq!(
            ...
            false,
            "same height tree, blocker, emitter"
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            true,
            "goes 1 over and hits higher tree"
        );
        assert_eq!(
            fixture(vec![(false, Some(1)), (false, Some(1)), (true, Some(2))]),
            false,
            "gets blocked"
        );
        assert_eq!(
            fixture(vec![(false, Some(1)), (false, Some(2)), (true, Some(3))]),
            false,
            "gets blocked"
        );
        assert_eq!(
            fixture(vec![(false, None), (false, Some(0)), (true, Some(3))]),
            true,
            "seed does not block"
        );
        assert_eq!(
            fixture(vec![(false, None), (false, Some(0)), (true, Some(0))]),
            false,
            "0 height doesn't emit"
        );
        assert_eq!(
            fixture(vec![
                (false, Some(0)),
